...
**Fase 7F (Staging)** ha sido validada mediante *smoke test global y aceptación humana*: Vercel/Railway/MariaDB separados para staging, configuración de DNS/TLS, CMS verificado, API, Legal, Contacto (formulario/persistencia) y Escuela (inscripciones, permisos) validados. El envío **SMTP real desde Railway queda BLOQUEADO por el plan Railway Hobby**; 7G.1B integra Resend por API HTTPS y ha validado con éxito operativamente en staging la recuperación de contraseñas. Sin embargo, para producción faltan dominio/remitente, secret y prueba controlada. Por tanto, la notificación real de Contacto, el flujo completo de identidad de menores y la recuperación de contraseña en producción siguen pendientes. La verificación de backups/restore/rollback en staging continúa **aplazada / no ejecutada**. 7G.1C corrige la premisa histórica: Railway documenta backups manuales y programados para volúmenes sin reservarlos a Pro; la estrategia en capas está lista, pero falta el restore lógico aislado y el rollback rehearsal. Se ha registrado una posible mejora de UX para `/aprende-a-jugar` (no bloqueante).
...
1. **Fase 7F (Pendientes de Producción):** configurar dominio/remitente y secret propios y realizar smoke test productivo para reset de contraseña; mantener cerrada la identidad pública completa de menores hasta su gate; y ejecutar el restore lógico aislado y rollback rehearsal de la estrategia 7G.1C en staging. SMTP continúa bloqueado en Hobby y no es la solución seleccionada.
...
el entorno de staging ha sido desplegado y validado, salvo el bloque de backup/restore/rollback (estrategia 7G.1C reconciliada, ensayo aún aplazado) y el correo real (SMTP bloqueado por Railway Hobby; Resend HTTPS integrado y probado en staging en 7G.1B, aún pendiente de configuración y prueba productiva, afectando a Contacto, menores y contraseñas); Railway permite actualmente backup manual y programado de volúmenes en Hobby, pero esa capacidad no sustituye el dump lógico portable ni la copia independiente de media, y
