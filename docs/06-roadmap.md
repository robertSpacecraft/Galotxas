...
**Fase 7F (Staging)** ha sido validada mediante *smoke test global y aceptación humana*: Vercel/Railway/MariaDB separados para staging, configuración de DNS/TLS, CMS verificado, API, Legal, Contacto (formulario/persistencia) y Escuela (inscripciones, permisos) validados. El envío **SMTP real desde Railway queda BLOQUEADO por el plan Railway Hobby**; 7G.1B integra Resend por API HTTPS y ha validado operativamente en staging la recuperación de contraseñas. En 7G.3 se configuró y validó la infraestructura productiva (dominio verificado, SPF/DKIM, API key separada en Railway). Queda pendiente la prueba real controlada (smoke productivo 7G.6) para la recuperación de contraseña. Las notificaciones reales de Contacto y menores siguen pendientes. El restore lógico de staging se completó exitosamente (PASS) durante 7G.1D (RTO 5m27s). 7G.1C corrige la premisa histórica comprobando operativamente que Railway restringe los backups nativos a Pro; la estrategia en capas externa está lista, pero falta el rollback rehearsal. 7G.1D validó operativamente el restore lógico aislado de MariaDB para staging cerrando su P0 (RTO ~5.5m), quedando pendiente la prueba de rollback y el gate de media/producción. Se ha registrado una posible mejora de UX para `/aprende-a-jugar` (no bloqueante).
