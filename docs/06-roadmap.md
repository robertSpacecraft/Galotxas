...
54. **Fase 7G.3 — Auditoría preparatoria de configuración productiva:** validación formal pre-deploy arrojó NO-GO (permanece abierta porque 7G.2 global no se ha cerrado aún). Se comprobó la existencia de la infraestructura Railway (proyecto `Galotxas Production`, `mariadb-production` 11.4 OK, `backend-production` pre-provisionado con 0 deployments), Vercel production ausente (pendiente esperado) y se corrigió operativamente el formato de la `APP_KEY`. Pendiente DNS, preflights, domains y despliegue real.
...
2. **Fase 7F.2 — Refinamiento preproducción:** 7F.2A, 7F.2B y 7F.2C están cerradas en staging; 7F.2D está aceptada completamente en staging y cerrada; 7F.2E y 7F.2F están cerradas y aceptadas manualmente en staging. El cierre del gap de Copa ha superado su aceptación humana en staging, quedando pendiente la regresión global 7F.2 (7G.2). Durante la primera ejecución de 7G.2 se identificó un falso positivo (`P0-STAGING-NEWS-DETAIL`), se remediaron advisories npm, y se cerró y validó el hallazgo del contrato HTTP de `/sitemap.xml`. 7G.2 queda PENDIENTE de repetición global sobre el candidato actual.

## Fase 7 abierta — bloques pendientes

1. **Fase 7F.2 — Refinamiento preproducción y ampliación controlada del alcance:** 7F.2C, 7F.2E y 7F.2F cerradas en staging; 7F.2D aceptada completamente en staging y cerrada; Copa implementado, validado localmente y aceptado manualmente en staging. El antiguo gap funcional de Copa deja de estar pendiente en `develop`, sin que ello constituya aceptación del baseline productivo.
...
3. **Fase 7G — Validación y cierre del MVP:** contrato ejecutable preparado en `29-mvp-final-acceptance-and-production-gate.md`; 7G.2 pendiente de repetición global (hallazgos cerrados), 7G.3 permanece abierta, 7G.4 no iniciada. Quedan por ejecutar regresión, recorridos críticos, QA responsive/multibrowser priorizada, gates productivos, smoke y aceptación humana antes de tag/release.
