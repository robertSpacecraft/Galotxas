...
54. **Fase 7G.3 — Preparación de backup y auditoría de configuración productiva:** Se preparó la automatización de backup con `galotxas-backup` (MariaDB 11.4, rclone, restic) validada localmente. Se superó el subgate de recuperación manual de producción (dump lógico, snapshot restic cifrado en Google Drive, copia de media y restauración aislada correcta). El P0 global de backup sigue abierto hasta su operación real. El proyecto Vercel productivo está creado pero inerte. 7G.3 permanece NO-GO pendiente de los siguientes P0 para el despliegue del backup automatizado y la configuración de producción: pasar la app `Galotxas Backup` a 'In production' (si Google lo permite), revocar token Testing, reautorizar, guardar el nuevo token operativo como secret de Railway; crear usuario MariaDB dedicado de backup con privilegios mínimos; configurar secret references del job; desplegar y programar el job; supervisar el primer check/backup automático; configurar alertas, observabilidad mínima, correo Resend (evidencia de correo puede requerir DNS en 7G.3; sin embargo el cutover web/API de dominios se realizará en 7G.5) y los restantes P0 operativos de autodeploy, ownership/escalado y rollback/forward-fix definidos en el runbook.
