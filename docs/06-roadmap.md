...
51. **Fase 7G.1B — Integración local de Resend/password-reset:** SDK oficial fijado, preflight Resend/`array` fail-closed, fallo de transporte no enumerable, token fallido invalidado y regresión backend completa validados sin envío real, secret, DNS o entorno remoto. El P0 continúa abierto hasta la prueba extremo a extremo controlada en staging.
...
1. **Fase 7F (Pendientes de Staging y Producción):** configurar dominio/remitente y secret propios y probar Resend extremo a extremo para reset de contraseña; mantener cerrada la identidad pública completa de menores hasta su gate; y ejecutar backup/restore/rollback en staging (aplazado). SMTP continúa bloqueado en Hobby y no es la solución seleccionada.
...
el entorno de staging ha sido desplegado y validado, salvo el bloque de backup/restore/rollback (aplazado) y el correo real (SMTP bloqueado por Railway Hobby; Resend HTTPS integrado localmente en 7G.1B, aún sin dominio/secret ni prueba real, afectando a Contacto, menores y contraseñas); el backup nativo de volúmenes también está bloqueado por el plan Railway Hobby, pero
