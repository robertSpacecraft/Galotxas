...
54. **Fase 7G.3 — Preparación de backup y auditoría de configuración productiva:** Se preparó la automatización de backup con `galotxas-backup` (MariaDB 11.4, rclone, restic). Se superó el subgate de recuperación manual de producción (dump lógico, snapshot cifrado, copia de media y restauración correcta). El P0 global de backup sigue abierto hasta su despliegue y operación real. El subbloque de la app OAuth `Galotxas Backup` ha sido resuelto: la app está 'En producción', se configuró y validó exitosamente una nueva autorización productiva con el alcance mínimo `drive.file` limitando el acceso a un nuevo repositorio (`rclone:gdrive-galotxes-backup:galotxes-backup-drivefile/production`), y la implementación versionada (`backend/backup/backup.sh`) se ha corregido para imponer `scope = drive.file` sin fallback. El antiguo repositorio y su snapshot manual pre-migración quedan preservados pero fuera de la retención automática. 7G.3 permanece NO-GO pendiente de los siguientes P0 para el despliegue del backup automatizado y la configuración de producción: configurar el job en Railway con el nuevo token operativo y secret references seguras; crear usuario MariaDB dedicado de backup con privilegios mínimos; ejecutar el modo `check` desde el job real; supervisar el primer backup automatizado; programar horario UTC; configurar alertas y observabilidad; correo Resend (evidencia de correo puede requerir DNS en 7G.3; sin embargo el cutover web/API de dominios se realizará en 7G.5) y los restantes P0 operativos de autodeploy, ownership/escalado y rollback/forward-fix definidos en el runbook. El proyecto Vercel productivo está creado pero inerte.
...
**Fase 7F (Staging)** ha sido validada mediante *smoke test global y aceptación humana*: Vercel/Railway/MariaDB separados para staging, configuración de DNS/TLS, CMS verificado, API, Legal, Contacto (formulario/persistencia) y Escuela (inscripciones, permisos) validados. El envío **SMTP real desde Railway queda BLOQUEADO por el plan Railway Hobby**; 7G.1B integra Resend por API HTTPS y ha validado con éxito operativamente en staging la recuperación de contraseñas. Sin embargo, para producción faltan dominio/remitente, secret y prueba controlada. Por tanto, la notificación real de Contacto, el flujo completo de identidad de menores y la recuperación de contraseña en producción siguen pendientes. El restore lógico de staging se completó exitosamente (PASS) durante 7G.1D (RTO 5m27s). 7G.1C corrige la premisa histórica comprobando operativamente que Railway restringe los backups nativos a Pro; la estrategia en capas externa está lista, pero falta el rollback rehearsal. 7G.1D validó operativamente el restore lógico aislado de MariaDB para staging cerrando su P0 (RTO ~5.5m), quedando pendiente la prueba de rollback y el gate de media/producción. Se ha registrado una posible mejora de UX para `/aprende-a-jugar` (no bloqueante).
...
3. **Fase 7G — Validación y cierre del MVP:** contrato ejecutable preparado en `29-mvp-final-acceptance-and-production-gate.md`; 7G.2 = PASS / CERRADO, 7G.3 permanece abierta como siguiente gate, 7G.4 no iniciada. Quedan por superar gates productivos, smoke y aceptación humana antes de tag/release.
