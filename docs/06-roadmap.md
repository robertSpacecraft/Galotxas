...
1. **Fase 2B.2 — Campeonatos:** corregir de forma aislada su contrato administrativo verificado por la auditoría.
2. **Fase 2B.3 — Categorías:** corregir de forma aislada su contrato administrativo verificado por la auditoría.
3. **Fase 2B.4 — Política pública de visibilidad:** definir y aplicar los criterios públicos de competición sin delegarlos en React.
4. **Fase 2B.5 — Contratos y API administrativa:** revisar los endpoints administrativos heredados como un bloque independiente.
5. **Reestructuración de navegación:** implantar Inicio, Competición, Aprende a jugar, Escuela de Galotxas y Club, conservando una migración compatible.
6. **Landing Competición:** agrupar Torneos, Rankings, Calendarios, Clasificaciones, Resultados y accesos de jugadores sobre contratos verificados.
7. **Landing Aprende a jugar:** crear la entrada divulgativa diferenciada del Manual.
8. **Contrato editorial de `knowledge/`:** normalizar metadatos, IDs, slugs, relaciones y validaciones de las colecciones aprobadas.
9. **Compilador build-time:** validar `knowledge/` y generar artefactos seguros y deterministas para React, sin MDX ni HTML ejecutable.
10. **Manual MVP:** construir la experiencia pública desde los artefactos generados, sin base de datos, API Laravel o CRUD Blade.
11. **Escuela de Galotxas:** combinar contenido pedagógico estable con actividad operativa administrable y protección específica de menores.
12. **Club y migración de Contenidos:** asignar una fuente canónica a cada página institucional y retirar gradualmente la arquitectura legada.
13. **QA, accesibilidad y despliegue:** validar contratos, recorridos, responsive, teclado, multimedia, persistencia y operación.

Estos bloques permanecen pendientes. Las rutas conceptuales y capacidades descritas no están implementadas por aparecer en el roadmap. Los permisos editoriales granulares, trazabilidad, preview, revisiones, redirects, noticias, actividades y uploads tampoco forman parte de la Fase 2B.1 y requerirán decisiones posteriores dentro de sus verticales correspondientes.
