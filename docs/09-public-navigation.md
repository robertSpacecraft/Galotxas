...
Escuela de Galotxas es una sección distinta del `academy` legado y del Manual. Su parte estable podrá incluir metodología, ejercicios y recursos pedagógicos desde una colección futura de `knowledge/`, únicamente cuando exista contenido aprobado. Fase 6B.1 implementa el dominio Laravel y Blade para programa permanente, niveles, horarios y ubicaciones, con visibilidad efectiva interna; todavía no existe lectura pública. Las solicitudes requieren 6B.2 y la API pública, 6B.4. El formulario no exigirá cuenta, creará una solicitud pendiente y sólo estará operativo cuando el backend confirme inscripciones abiertas. Centros y actividades seguirán siendo administrativos en el MVP y se implementarán en 6B.3. Un aviso simple podrá permanecer en el CMS si no necesita estructura.
