...
- **5.7-E — INTEGRIDAD DE CATEGORYENTRY (IMPLEMENTACIÓN COMPLETA / PENDIENTE DE ACEPTACIÓN OPERACIONAL):** Una entrada referencia exactamente un jugador o un equipo, coherente con su tipo, y cada identidad aparece como máximo una vez por categoría con independencia de `status`. `CategoryEntryService` concentra la escritura bajo el guard `PARTICIPANTS` y el orden de locks canónico: XOR y tipo, duplicados, modalidad del campeonato, inscripción `approved`, y para equipos, misma categoría y dos jugadores distintos delantero y zaguero. `POST /api/v1/admin/categories/{category}/entries` conserva ruta y respuesta, valida con `StoreCategoryEntryRequest`, devuelve `422` controlado —también ante carreras detectadas por la base de datos— sin SQL, mantiene el `409` de resultados oficiales y crea siempre entradas `approved`. En individuales, la inscripción sólo reutiliza una entrada previa exactamente coherente y `approved`; cualquier otra entrada existente aborta la inscripción con un error controlado, sin promoverla, sustituirla ni duplicarla, y revierte la inscripción recién creada. Las altas y bajas de inscripción de dobles usan el mismo guard y locks, y no puede retirarse la inscripción de un jugador que pertenece a un equipo de la categoría. Cambiar el `type` de un campeonato se rechaza mientras existan inscripciones, entradas o equipos, sin convertir ni borrar datos. La migración forward-only `2026_09_24_000000_enforce_category_entry_identity_integrity` aborta con diagnóstico antes de cualquier cambio si hay datos heredados incoherentes y añade en un único `ALTER` el `CHECK` `category_entries_identity_check` y los `UNIQUE` `category_entries_category_player_unique` y `category_entries_category_team_unique`; MariaDB 11.4.10 aceptó el `CHECK` sobre columnas con claves foráneas `ON DELETE CASCADE` y sustituye el índice implícito de `category_id` por el `UNIQUE` compuesto. No cambian las acciones `ON DELETE`, `status`, React ni los contratos públicos. Evidencia local previa a la corrección de revisión 1: suite backend completa 1.898 tests/18.773 aserciones en MariaDB aislada, con 82 tests nuevos (API, flujos Blade, cambio de tipo, migración, servicio y carreras con procesos reales) y tres casos de readiness reescritos con entradas en memoria; `php -l`, Pint afectado y `git diff --check` en PASS. Tras esa corrección, los focales directamente afectados pasaron 78 tests/305 aserciones, y `php -l`, Pint sobre los archivos tocados y `git diff --check` volvieron a pasar; la suite backend completa posterior a la corrección está pendiente y no se atribuye un resultado completo final hasta entonces. Pendiente: revisión humana, suite backend completa posterior a la corrección, aceptación en staging y migración remota tras sondas de solo lectura autorizadas. Deuda de seguimiento: la regla funcional de inscripción vigente —un jugador no puede estar asignado a dos categorías del mismo campeonato— se comprueba sólo en la aplicación y su serialización concurrente entre las categorías del campeonato sigue sin resolverse. Ver ADR-057.
- **MINOR-PUBLIC-IDENTITY-DIRECT-1 (CLOSED / PASS):** Los commits funcional `6fc2ed1` y documental `4d56b61` validan operativamente la autorización administrativa de la identidad pública verificable de menores (modos `alias` o `name_initial`), solicitada de forma directa desde la ficha de un jugador menor preexistente. Requiere la confirmación por correo del representante bajo un aviso versionado exacto, y para edades de 14–17 años se registra administrativamente la conformidad informada del menor antes de su aprobación final. La proyección pública es fail-closed y revocable. El correo privado identifica al menor registrado omitiendo su DOB, DNI, IDs internos o alias. La entrega real de correo en staging se completó exitosamente tras configurar el mailer desde `array` a `resend`. En producción, el código está desplegado y los flags de autorización y notificación han sido activados. La gate de identidad privada en contextos autenticados sigue abierta.
- **CATEGORY-COMPETITION-PDF-VISUAL-1 (CLOSED / PASS):** El commit `97706d186f4946f280e2760c7d4f413b71b8b741` aplica el refinamiento visual de la exportación live de categoría en PDF 6.F.3D.1 (no snapshots de entrega). Mejora la agrupación visual de jornadas y fases deportivas conservando la estricta paginación de una única página A4. Las celdas de partidos sin resultado quedan en blanco para facilitar la anotación manual en papel, mientras los estados explícitos aplazado o cancelado se mantienen visibles. Superó la regresión focal y completa del backend antes del commit, con aceptación humana en staging y despliegue productivo. No requiere migración.
- **PROFILE-SELF-SERVICE-1 (CLOSED / PASS):** En los commits `8c10b853318c217664dd1fbee8a0fe2c8d8c5a57`, `ceac94aa39b6017bfc11981a525881bc3efa4212`, `2aca545c07d3d5bef7166771e6782660ff7ec3f1` y `ba9e6859dd923fe94b8fed6d50894431c49d87ae`, se añade el perfil editable self-service y la integridad de sus campos. Se refuerza la unicidad nullable del apodo (nickname) y el control de colisiones tras normalización. Se implementa la integridad de la fecha de nacimiento (DOB) con protección fail-closed en torno a menores conocidos y a la evidencia de autorizaciones preexistentes, así como el guardado de declaraciones versionadas. Todo ha sido aceptado en staging, desplegado en producción y las migraciones productivas requeridas han sido completadas.
- **5.7-C — HARDENING DE LA API DE REPROGRAMACIONES (CLOSED / PASS):** El commit funcional `2df9da1263fa4b12e98a4f15b1a761d607046f3d` (`fix(api): endurecer reprogramaciones de partidos`) endurece los tres endpoints autenticados de reprogramación sin cambiar su dominio. Añade `SubmitMatchRescheduleRequest` y `ConfirmMatchRescheduleRequest`, fecha estricta `Y-m-d` entre `1000-01-01` y `today()->addYearsNoOverflow(2)`, hora `H:i`, pista existente, comentario opcional de hasta 2.000 caracteres y mensajes explícitos en castellano; las fechas históricas válidas siguen admitidas. El workflow y la confirmación usan `ParticipantMatchResource`, mientras `MatchRescheduleRequestResource` queda contraído a `side`, fecha propuesta, estado, comentario y pista `{id, name}`, sin IDs técnicos, timestamps, usuario, jugador, email ni trazabilidad del actor; también se retiraron eager loads innecesarios. Se alinea `can_submit=true`, `can_confirm=false` y `blocked_reason=null` para que el autor actualice su propuesta `submitted` mientras no exista una rival, sin reabrir solicitudes validadas. El limiter `match.reschedules` comparte diez intentos/minuto por usuario/IP entre ambos `POST`, conserva cabeceras estándar y envelope `429`, y no afecta al `GET`. Se preservan perfiles y lados singles/doubles, locks y transacciones, unicidad partido/lado, bloqueo de compañero, confirmación rival, colisión exacta dentro del mismo campeonato, recheck de ocupación y `OfficialResultMutationGuard`, sin estados nuevos. Evidencia: focal final post-Pint 44 tests/443 aserciones, `ApiRateLimitingTest` 7/57, `OfficialResultMutationGuardTest` 27/87, regresión dirigida 116/1.091 y suite backend completa 1.764/17.946 en MariaDB aislada (134,14 s), todo PASS; la dirigida y la completa precedieron a una corrección exclusivamente estilística de imports/tipo en un test nuevo y no se atribuye una suite completa posterior. También pasaron `php -l` sobre diez PHP, Pint focal, `git diff --check`, `git diff --cached --check` y auditoría humana exacta; no se limpió la deuda Pint global. Railway staging desplegó el SHA exacto con SUCCESS, arranque/readiness, caches, healthcheck y HTTP `200`; el usuario lo aceptó y autorizó la promoción, pero no se ejecutó ni se atribuye un walkthrough manual paso a paso de la API. `main` recibió el commit por fast-forward sin merge commit. Railway producción desplegó el mismo SHA con SUCCESS, build/push, caches de rutas y Blade, PHP-FPM ready, `/up` y HTTP `200`; su evidencia es sólo despliegue/readiness/salud, sin smoke funcional de reprogramación ni datos fabricados. No hubo migración, cambio frontend/React, contrato público de resultados ni cambio de los Resources de partido administrativo o de participante; 5.7-F conserva la ocupación física compartida/concurrencia, 5.7-D la UI React y la identidad privada/de menores su gate separada. El siguiente bloque canónico es **5.7-E — Integridad de `CategoryEntry`**.
- **5.7-B — PRIVACIDAD DEL RECURSO DE PARTIDOS DE PARTICIPANTE (CLOSED / PASS):** El commit funcional `1a628f2120e911daaea6f36bbc9003471c600271` (`fix(api): minimizar recursos de partidos de participante`) sustituye el `MatchResource` amplio por `ParticipantMatchResource` en `GET /api/v1/me/matches` y en los partidos de `GET /api/v1/me/calendar`, con allowlists exactas. Ambas respuestas dejan de incluir `result_reports`, comentarios y email del reportante, `submitted_by`/`validated_by` y sus objetos de usuario, claves foráneas internas, timestamps y metadatos editoriales exclusivos del Resource heredado. La consulta de participante ya no carga `resultReports.user`, `resultReports.player.user`, `submittedBy` ni `validatedBy`, y `pending-actions` carga por sí solo `resultReports`. Un tanteo almacenado en un partido `submitted` —estado que la administración puede producir— queda oculto hasta que el partido está `validated`; los tanteos validados siguen visibles. La visibilidad pública de los resultados validados no cambia, y el `MatchResource` y el `MatchResultReportResource` administrativos, `PublicMatchResource` y las reprogramaciones —propiedad posterior de 5.7-C— permanecieron sin cambios en aquel cierre. La evidencia automatizada fue `ParticipantMatchPrivacyTest` con 10 tests y 155 aserciones tras un endurecimiento final exclusivamente de tests (10 y 152 antes de él), regresión backend dirigida de 115 tests y 1.269 aserciones, suite backend oficial de 1.719 tests y 17.482 aserciones sobre MariaDB aislada y 21 tests frontend dirigidos, todos en PASS, además de `php -l`, Pint afectado y `git diff --check`. Las ejecuciones dirigida y completa son anteriores a ese endurecimiento final y la suite completa no se repitió por tratarse de un cambio sólo de tests. Staging desplegó el SHA exacto con healthcheck correcto y la aceptación humana fue PASS. Producción desplegó el mismo SHA con estado SUCCESS, arranque correcto y salud HTTP `200`, pero carece de datos de competición representativos: su evidencia es exclusivamente de despliegue y salud, no un smoke funcional, y no se fabricaron datos. No hubo migración, cambio de configuración ni cambio de código React. La gate de producto/privacidad sobre la identidad privada de participantes y de menores en contextos autenticados permanece abierta y fuera de este bloque. 5.7 permanece abierto; en aquel cierre el siguiente bloque canónico era 5.7-C y su cierre actual queda registrado en la entrada superior.
* **Actualización de textos legales públicos:** se actualizan el Aviso Legal a la versión `1.1.0` y la Política de Privacidad a la versión `1.2.0` con los textos revisados vigentes desde el 19/09/2026. Se regenera la proyección legal consumida por el frontend y se adaptan las pruebas a la nueva estructura documental. La Política de Cookies y los avisos específicos de formularios permanecen sin cambios. Validación en PASS mediante `legal:build`, `legal:check`, 21 tests unitarios dirigidos y 6 E2E legales ejecutados con el runner Docker oficial.
- **5.7-A — VALIDACIÓN ESTRICTA DE FECHA EN ADMINISTRACIÓN DE PARTIDOS (CLOSED / PASS):** Los commits funcionales `1436c1d671b91640e8d78b03874aac8257b43e70` y `bc37983664220c7ddb2c43e69bb3954d14f9137f` validan `scheduled_date` antes de construir Carbon con una fecha real y canónica `Y-m-d`, mínimo técnico `1000-01-01` y máximo funcional dinámico `today() + 2` años naturales sin desbordamiento. Los formularios repetidos de Liga y Copa reflejan esos límites y muestran dinámicamente un aviso no bloqueante para fechas anteriores a `today() - 5` años; las fechas históricas válidas siguen admitiéndose y los errores de fecha usan mensajes explícitos en castellano. La regresión final fue 14 tests/132 aserciones focales, 54/286 dirigidos y 1.709/17.331 en la suite backend oficial sobre MariaDB aislada, además de checks de sintaxis, Pint afectado y diff en PASS. La aceptación manual completa de staging fue PASS y el código fue promovido a `main`. Producción no dispone de datos de competición representativos para repetir el smoke manual dependiente de partidos; por decisión humana no se fabricaron datos ni se atribuye esa comprobación no realizada. No hubo migración, cambio de API pública, React, reprogramaciones ni refactor del controlador. 5.7 permanece abierto; en aquel cierre el siguiente bloque canónico era 5.7-B y su cierre actual queda registrado en la entrada superior.
- **P1.D.3 / P1.D — CIERRE OPERACIONAL RESPONSIVE (CLOSED / STAGING PASS):** Tras readiness audit, descubrimiento read-only y confirmación explícita del freeze de writers, staging ejecutó exactamente un canary APPLY autorizado sobre `news#1` (`--domain=news --after-id=0 --limit=1`). El run `cf2a56f9-6af0-400d-8af6-55304aa2544b` terminó `completed`, outcome `success` y exit 0, con un item publicado, cuatro variantes y un manifest creados; el journal posterior quedó en runs/items/objects/events `1/1/5/0`, Barrier V2 `clear`, sin ambigüedad ni cleanup, y el dry-run reclasificó la referencia como `responsive_ok`. La reconciliación posterior fue read-only y no mutó journal o storage; mantenimiento se conservó hasta terminar los checks y la imagen recibió aceptación visual humana tras restaurar staging. El canary acredita sólo el camino normal bajo el freeze auditado: no es un backfill completo, no implementa forward reconciliation, cleanup, ausencia confirmada o `--resume`, y no habilita mutación automática. No hubo retry, cleanup, reconciliación mutante, APPLY o mutación de datos en producción; una migración productiva futura requiere autorización e inventario/freeze/gates nuevos y no bloquea el cierre de P1.D. El siguiente bloque canónico es 5.7 — Hardening P1/P2 vigente.
- **6.C — PORTADAS OPCIONALES DE COMPETICIÓN (CLOSED / PASS):** Los commits funcionales `744b10eb98cf13098f1e68706b372fe52526a866` y `97d11a52acc9aa5143d706d2acae9c0af4c8d4f8` incorporan una portada propia, opcional e independiente para cada Temporada, Campeonato y Categoría. Backend y Blade reutilizan la multimedia privada persistente con perfil `banner`, lifecycle con compensación y cleanup posterior al commit, preview administrativo autorizado y rutas públicas estables; la API sólo expone `image: { "url": "..." }` o `null`, nunca keys ni referencias de storage. React presenta las portadas en `/competicion`, tarjetas y detalle de Campeonatos y resumen de Categoría, con layout estable, carga diferida por defecto y ocultación fail-soft; Clasificación, Calendario y resultados, y Copa quedan fuera y no existe herencia entre niveles. 6.C.1 acreditó 820 tests/6.698 aserciones de backend y 127/1.337 focales; 6.C.2, 708/708 tests frontend y 76/76 focales, además de lint, build, checks acotados y aceptación humana local, staging y producción. Producción aceptó el comportamiento disponible, aunque su dataset real no permite recorrer todos los escenarios dependientes de datos; no se declara E2E global ni se reabre la mejora P1 separada de variantes responsive, compresión y entrega.
- **6.F.3D.1 — EXPORTACIÓN PDF DE COMPETICIÓN POR CATEGORÍA (CLOSED / PASS):** El commit funcional `903eefce4d573f2447a0891dde914bf2a43ff6a9` añade al detalle administrativo de categoría una descarga directa con el estado `live` completo de Liga y Copa en un PDF binario A4 portrait de exactamente una página. El builder proyecta identidad pública sin PII y sólo publica tanteos `validated` coherentes; el renderer Dompdf 3.1.6 prueba los presets `standard`, `compact` y `dense`, cuenta páginas y falla cerrado ante overflow sin recortar partidos o nombres ni persistir el documento. La validación acreditó 14 tests/106 aserciones focales, 177/1.384 dirigidos y 689/5.266 de backend sobre MariaDB, fixture 10 participantes/45 Liga/4 Copa, inspección independiente A4, benchmark dentro de gates y aceptación humana local, staging y producción. La exportación es operativa y actual: no consume snapshots oficiales ni constituye un certificado histórico. El refinamiento visual fue cerrado posteriormente por CATEGORY-COMPETITION-PDF-VISUAL-1 en el commit `97706d1`.
- **6.F.3D — OFICIALIZACIÓN Y REAPERTURA DE LIGA (CLOSED / PASS):** El commit funcional `04231bd1368c10303476efd4b76f4edddf1bb437` incorpora un lifecycle service-only y fail-closed para convertir una Liga completa en evidencia oficial reproducible: readiness estructural y deportiva, cálculo compartido con el ranking vivo, snapshot versionado de clasificación y partidos, identidad pública minimizada a fecha de oficialización, actor y motivo de reapertura preservados, y digest SHA-256 canónico `league-source-v1`. La secuencia `v1 official → v1 reopened → v2 official` conserva intacta cada versión y usa el mutex común por `Category`; un oficial vigente de Copa puede coexistir con la oficialización o reapertura de Liga, pero sigue protegiendo los writers dependientes. La validación obtuvo 126 tests/1.067 aserciones dirigidos, 675/5.160 de backend y siete carreras E2E sobre MariaDB real, además de lifecycle humano local y aceptación técnica y humana en staging y producción. No hubo migración ni se añadieron controller, request, route, Blade, API o React: la administración queda para 6.F.3F y el contrato público para 6.F.3G. Los entornos remotos no ejecutaron lifecycle ni acreditaron delta PRE/POST: staging conservó dos muestras POST iguales y producción, sin dataset deportivo, conservó también dos muestras POST iguales; no se fabricaron datos. Pint del diff fue PASS, mientras las 23 incidencias globales preexistentes y los dos avisos `npm audit` del frontend permanecen como deuda independiente.
- **6.F.3C — LOCKING COMÚN Y PROTECCIÓN DE MUTACIONES (CLOSED / PASS):** El commit funcional `82d5c35b3c1b20fe6595c4d25f775298b765a154` introduce un mutex común por categoría y ordena de forma determinista los locks de `Category`, resultados oficiales vigentes —Liga antes que Copa—, rondas, partidos, entradas y equipos dentro de una misma transacción. Los mutation guards bloquean resultado o estructura de Liga ante un oficial vigente de Liga o Copa, las mutaciones decisivas de Copa ante un oficial vigente de Copa y cualquier cambio de participantes ante cualquiera de las dos partes; el tercer puesto queda fuera de v1 y una clasificación estructural ambigua falla de forma cerrada. Los borrados protegidos combinan precheck de dominio con `RESTRICT`, sin bypass administrativo ni force-delete. Blade devuelve un mensaje comprensible y la API administrativa un `409` con `{ "message": "...", "data": null }`, mientras los estados administrativos, visibilidad y metadatos no deportivos continúan independientes. La validación obtuvo 39 tests/218 aserciones focales, 168/1.265 dirigidos y 641/4.844 de backend, Pint del diff, `php -l` y `git diff --check`, además de aceptación automática y humana local, en staging y en producción. No hubo migración nueva ni se añadieron servicios de oficialización/reapertura, UI o endpoint público. Producción está desplegada y conserva las nueve tablas deportivas implicadas a cero; no se fabricaron datos para reproducir el bloqueo condicionado y esta limitación no impide el cierre.
- **6.F.3B — PERSISTENCIA/VERSIONADO DE RESULTADOS OFICIALES (CLOSED / PASS):** `CategoryOfficialResult` persiste por categoría y parte (`league` o `cup`) versiones con lifecycle `official`/`reopened`, historial inmutable y un único resultado vigente por categoría y parte mediante `current_slot` generado e índice único. Cada versión congela sus partidos y, según corresponda, la clasificación completa de Liga o exclusivamente el campeón de Copa; las proyecciones de identidad separan el nombre histórico interno del nombre público y excluyen datos personales sensibles. Las referencias de origen actúan como evidencia sin dependencia destructiva, la categoría queda protegida por `RESTRICT`, los actores pueden desaparecer con `SET NULL` conservando sus nombres snapshot y sólo los hijos técnicos dependen por `CASCADE` de su versión. La migración fail-closed no hace backfill, preserva cualquier tabla preexistente y sólo limpia tablas marcadas que haya empezado a crear en la misma ejecución. El commit funcional `bf698708712a6682fa4a7faf6b8ce22defa1fd98` superó 28 tests/173 aserciones del dominio, 4/92 de migración, 129/1.245 dirigidos y 613/4.755 de backend, además de Pint, `php -l` y `git diff --check`; staging y producción completaron backup/restore, migración manual única, verificación de esquema, smoke y aceptación humana con backfill cero. En aquel cierre aún no existían servicios de oficialización/reapertura, mutation guards, cálculo de digest, administración, endpoint API ni presentación React; 6.F.3C incorporó posteriormente los guards, como registra la entrada superior.
- **6.F.3A — CICLO DE VIDA DE TEMPORADA ACTIVA ÚNICA (CLOSED / PASS):** `SeasonStatus` queda alineado con `planned`, `active`, `finished` y `cancelled`, y el default histórico `pending` pasa a `planned`. Backend y MariaDB admiten cero o una temporada activa y rechazan dos o más: `SeasonService` unifica las escrituras Blade/API, mientras la columna generada `active_slot = IF(status = 'active', 1, NULL)` y el índice único `seasons_one_active_unique` aportan la defensa final y traducen el conflicto a validación de dominio. Los estados `finished` y `cancelled` de temporada o campeonato siguen siendo operativos y no equivalen a un resultado oficial. El commit funcional `dc517f1635817ad63c60dd9ab8e0e1b493ff1624` superó 36 tests y 260 aserciones dirigidas, la suite backend de 585 tests y 4.582 aserciones, Pint acotado, `php -l` y `git diff --check`; staging y producción completaron backup/restore, migración manual, smoke y aceptación humana. Producción conserva su dataset real de cero temporadas, por lo que allí no se fabricaron datos para probar una segunda activa. En aquel cierre, el siguiente microbloque era **6.F.3B — Persistencia/versionado de resultados oficiales**; su cierre actual queda registrado en la entrada superior.
- **6.F.2 — JERARQUÍA DE `/competicion` (CLOSED / PASS):** `/competicion` prioriza visualmente todas las temporadas `active`, muestra sus campeonatos directamente, separa las `planned` y limita el histórico compacto a tres `finished` con enlaces a `/torneos?season_id={id}`. `/torneos` continúa como explorador/archivo compatible y sincroniza `season_id` con la URL sin descartar parámetros ajenos. El commit funcional `1cf5e687e482b31c7bfedd3e6f97ae2e87c2ccd4` superó 93 archivos y 676 tests frontend, ESLint, build Vite, smoke público 19/19, staging con datos y aceptación humana, y producción `READY`. La regresión E2E global observada en ese ciclo fue 67/68: el único fallo, un dropdown Blade del CMS administrativo que intercepta `Crear bloque`, es independiente y continúa abierto. Producción no contiene todavía temporadas, campeonatos ni rankings; el smoke dependiente de datos queda diferido hasta disponer de dataset deportivo real, sin que esta limitación conocida reabra 6.F.2.
- **P1-UX-MIPANEL-NAV-MOBILE (CLOSED / PASS):** Mi Panel sustituye la fila con scroll horizontal oculto por cinco pestañas siempre visibles: 2+2+1 hasta 480 px, 3+2 entre 481–900 px y una fila desde 901 px cuando cabe. El patrón accesible incorpora `tablist`/`tab`/`tabpanel`, relaciones ARIA, `roving tabindex`, foco visible y activación manual por teclado sin disparar lazy loading al mover el foco. El commit `09dc4b51f2eecd8155f31b1bbd3e8d880af285a7` superó Dashboard 5/5, E2E focalizado 1/1, 93 archivos y 680 tests frontend, ESLint, build y smoke Chromium en staging y producción. Las APIs privadas se interceptaron y no se ejercitaron mutaciones reales de formularios, foto o datos, por diseño no destructivo; la limitación del smoke no constituye un defecto conocido. En aquel cierre, el siguiente bloque oficial era 6.F.3 — dominio de resultados oficiales.
- **6.A — AGRUPACIÓN CMS DEL ADMIN (CLOSED / PASS):** El menú Blade agrupa ahora `Páginas` y `Navegación` bajo el desplegable `CMS`, sin añadir una ruta al padre ni cambiar las rutas o comportamientos existentes. Las dos familias conservan estados activos accesibles mediante `aria-expanded` y `aria-current="page"`; `AdminCmsMenuTest` incorpora su cobertura de regresión. La suite backend segura pasó con 571 tests y 4.524 assertions, y la validación humana fue PASS en local, staging y producción. No se añadieron modelos, controladores, API, dominio ni migraciones; el siguiente bloque oficial es 6.F.
- **6.B — HERO FOTOGRÁFICO DE HOME (CLOSED / PASS):** Home integra fotografía responsive mediante variantes WebP optimizadas para desktop y mobile, conserva el tratamiento azul en CSS, el H1 `Galotxes en Monóvar` y sus CTA funcionales. Se retiraron los PNG y el componente `Hero` legado huérfanos. El bloque superó validación automática y aceptación humana local, staging `READY` con aceptación humana desktop/mobile y despliegue y smoke productivos; 6.B queda cerrado y el siguiente bloque oficial es 6.A.
- **RECONCILIACIÓN DOCUMENTAL POST-MVP DE GATES Y PLAN:** Se cierran a nivel arquitectónico, sin implementar funcionalidad, las gates de i18n React castellano/valenciano y del sistema visual Liquid Glass con Light/Dark/System. Castellano (`es`) continúa como fuente y URL sin prefijo; valenciano (`ca-valencia`) usará `/va/` y slugs compartidos inicialmente. `/admin` permanece Blade, en castellano y fuera de i18n, Liquid Glass y dark mode. ADR-046/ADR-047, gobernanza, pipeline Knowledge, navegación y estilo frontend conservan estas decisiones. El plan definitivo post-MVP fija 6.B Hero fotográfico de Home como primer bloque —comenzando por auditar el asset, su enlace/renderizado y su idoneidad antes de seleccionar la imagen con el usuario— y 6.A agrupación CMS como segundo; Liquid Glass se aislará en `feature/liquid-glass` y exigirá piloto y aceptación humana. Este cambio es exclusivamente documental.
- **CIERRE DOCUMENTAL POST-RELEASE DE 7G.7 Y DEL MVP:** La autorización humana expresa fue concedida, 7G.7 queda PASS/CERRADO y Fase 7G y el MVP quedan CERRADOS/PUBLICADOS. La release estable [`v0.1.0`](https://github.com/robertSpacecraft/Galotxas/releases/tag/v0.1.0) existe y su tag anotado remoto apunta exclusivamente a `30a22844697e403d699926c2a5a0193f78a5bc71`. El hash `0b78552612231a9bfd450e96bf258f66c3192586` se mantiene como evidencia técnica histórica de 7G.3, no como hash de release. Esta reconciliación documental es posterior a la publicación y no forma parte del contenido etiquetado como `v0.1.0`.
- **EVIDENCIA PRODUCTIVA FINAL:** Producción permanece `live` + `indexing=true`; el fix SEO está desplegado, el sitemap ofrece 53 URLs y Search Console lo registra como correcto con 53 URLs descubiertas, sin afirmar que todas estén indexadas. Home está indexada, las URLs públicas seleccionadas superaron la prueba en vivo y `/login` conserva `noindex,nofollow`. PageSpeed obtuvo 96/100/100/100 en móvil y 100/100/100/100 en escritorio; `SportsClub` obtuvo 0 errores y 0 warnings. El reset real mediante Resend, el backup cifrado DB+media posterior al contenido y el smoke productivo 7G.6 finalizaron con PASS. Vercel producción despliega `main`, Vercel staging despliega `develop` y Railway backend despliega `main`; los filtros de build evitan previews cruzadas. El push documental `30a228...` quedó correctamente `SKIPPED` en Railway al no modificar `backend/**`.
- **EXCEPCIÓN Y CONTINUIDAD POST-MVP:** El rollback rehearsal remoto de 7G.3 NO FUE EJECUTADO y conserva su excepción humana histórica; no se reclasifica como PASS. Las prioridades P1/P2 permanecen como trabajo post-MVP y no bloquean la release publicada.
- **CIERRE DE 7G.3 (PASS CON EXCEPCIÓN HUMANA DOCUMENTADA) Y ENTRADA A 7G.4:** Se cierra formalmente el gate 7G.3 tras completar las validaciones pendientes y registrar la decisión humana final. El hash candidato exacto fijado y validado es `0b78552612231a9bfd450e96bf258f66c3192586`. El ensayo de rollback (rollback rehearsal) remoto **NO FUE EJECUTADO** por decisión humana explícita del operador único, asumiendo el riesgo residual para la primera publicación al no haber artefactos web reales previos en producción y basándose en la compatibilidad auditada con 7G.2 y mecanismos fallback identificados. Esta excepción no autoriza rollbacks de BD automáticos y la estrategia DB sigue siendo forward-only. Validaciones finales superadas: Backend (566 tests / 4.350 assertions, PHP lint 0 errores), Composer (strict y audit PASS), Backup tests (9/9) y Docker build, Frontend (`npm ci`, 660 tests Vitest, ESLint y audit PASS), Legal/Knowledge/SEO (PASS, noindex), `deploy:check`/`build` (PASS), y E2E aislado (68/68 Chromium). P1 documentado no bloqueante: Pint detectó 28 incidencias históricas de formato. La publicación `initial` + `noindex` avanza al siguiente gate: 7G.4 (GO PRODUCCIÓN explícito / CLOSED).
- **OPERACIÓN Y CONTROL SOURCE BACKUP 7G.3:** Se valida la operatividad productiva del automatismo de backup y se cierra el P0 de control de source/autodeploy mediante una política deliberadamente manual para la primera publicación. Se configuró el usuario dedicado `galotxas_backup` en MariaDB con mínimos privilegios. El P0 de autodeploy se resuelve desactivando el autodeploy de GitHub en `backend-production` (conectado a `main`) y manteniendo `backup-production` source-less (sin integración a GitHub), desplegado exitosamente vía `railway up` manual desde un árbol limpio de `develop`. Esta política exige acción humana explícita para cada despliegue. En la infraestructura de backup, se validó el servicio con secretos, retención, cron UTC y alertas. Se ejecutó exitosamente el modo `check` y el primer backup productivo supervisado, creando el snapshot `b7225bfc` sobre un esquema pre-migraciones, cuya integridad fue ratificada (SHA256 PASS) en restauración. El gate 7G.3 cerró con excepción documentada del rollback rehearsal; el hash técnico de ese gate fue validado. El P0 de ownership ha sido resuelto documentando una política formal de mantenedor único, asumiendo explícitamente el bus factor 1 como riesgo y estableciendo la congelación de despliegues y operaciones privilegiadas ante indisponibilidad (sin suplente interno ni escalado ficticio; el soporte de proveedor no sustituye el ownership).
...
- **AUDITORÍA PREPARATORIA 7G.3:** Se superó el drill manual de recuperación de producción. Se constató la infraestructura backend en Railway y se corrigió operativamente la `APP_KEY`. Se preparó y acreditó la infraestructura externa y credenciales para el correo productivo con Resend, difiriendo el test de entrega real al smoke post-despliegue (validado exitosamente a posteriori en 7G.6). El P0 de ownership fue cerrado aprobando la política de mantenedor único y bus factor 1. El proyecto Vercel productivo `galotxas` está inerte. El gate 7G.3 cerró con excepción.
...
- Semifinal, Final y tercer puesto usan `phase=cup` y stages explícitos; la
  generación valida exactamente dos semifinales validadas sin empate, conserva
...
- El schedule público existente añade fase, stage y `winner_entry` allowlisted,
  orden estable y tanteos sólo de partidos validados. React incorpora la ruta diferida
...
- La validación de 7F.1 pasa 431 tests backend, 493 frontend y 63 E2E, además de
  ...
  hashes. En aquel cierre técnico, 7F, 7G, Fase 7 y MVP permanecían abiertos;
  no hubo deploy ni activación en ese bloque.
...
- La operación se valida con 421 tests backend, 484 frontend y 63 E2E, además
  ...
  quedó cerrada técnicamente sin datos inventados ni activación; en aquel
  momento 7F, 7G, Fase 7 y MVP permanecían abiertos.
...
- Vite genera `robots.txt` y un sitemap determinista de 52 URLs sólo bajo una
  ...
  el cierre técnico de 7D. En ese momento, Fase 7, MVP, dominio y despliegue
...
- La reprogramación no tiene interfaz React; 5.7-C incorporó posteriormente el limiter específico compartido de sus escrituras.
