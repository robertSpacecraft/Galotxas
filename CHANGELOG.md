...
- **OPERACIÓN BACKUP RAILWAY 7G.3:** Se valida la operatividad productiva del automatismo de backup. Se configuró el usuario dedicado `galotxas_backup` en MariaDB con mínimos privilegios (USAGE global, SELECT y equivalentes). Se desplegó manualmente en Railway el servicio `backup-production` alimentado con secretos, retención y variables de entorno reales, programado mediante cron (UTC) y con alertas preexistentes validadas. Se ejecutó exitosamente en el job real el modo `check` y posteriormente el primer backup productivo supervisado, creando el snapshot `b7225bfc` sobre un esquema pre-migraciones. La integridad del backup fue ratificada con éxito (SHA256 PASS) en restauración. El P0 técnico global de source/autodeploy de este servicio se mantiene abierto ya que el despliegue actual fue manual. El gate 7G.3 permanece NO-GO por P0 operativos (ownership, rollback rehearsal, correo y hash).
