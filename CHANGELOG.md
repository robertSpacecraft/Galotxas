...
- **OAUTH Y BACKUP 7G.3 (DRIVE.FILE):** Se reconcilia el subbloque OAuth/Google Drive para backups automáticos (`fix(ops): limitar backups a Google Drive drive.file`). La App `Galotxas Backup` se pasa a 'En producción' (tipo External). Se utiliza exclusivamente el scope restringido `drive.file`. El repositorio remoto canónico cambia a `galotxes-backup-drivefile/production`, ya que el scope limitado impide leer el repositorio antiguo (`galotxes-backup/production`), el cual queda preservado intacto con su snapshot `7G.3-pre-migration`. El script `backend/backup/backup.sh` ha sido corregido para hardcodear `scope = drive.file` sin fallback y la prueba local confirma permisos de creación, lectura, escritura y restauración exactos. El snapshot de prueba (`31d9d05b`) en el probe tag no pertenece a la retención automática. El P0 de automatización real (job en Railway, secretos, usuario DB dedicado, validación de un modo check desde el job real, alertas y observabilidad) sigue abierto y 7G.3 permanece NO-GO.
- **AUTOMATIZACIÓN DE BACKUP 7G.3 PREPARADA:** Se añade una imagen efímera independiente del backend web con cliente MariaDB 11.4, rclone y restic; dump consistente, copia de media, manifiesto no sensible, limpieza garantizada, modo `check` y retención agrupada por `host,tags`. La solución se validó localmente con mocks e imagen real, limitando la integración Google Drive al scope `drive.file` y sin desplegar ni programar aún en Railway.
- **AUDITORÍA PREPARATORIA 7G.3:** Se superó el drill manual de recuperación de producción. Se constató la infraestructura backend en Railway y se corrigió operativamente la `APP_KEY`. El proyecto Vercel productivo `galotxas` está creado pero inerte. El gate 7G.3 permanece NO-GO pendiente de configurar/desplegar el job con el token operativo y usuario BD dedicado, primer backup/check automatizados supervisados, horario UTC, alertas, correo y demás P0 definidos en el runbook.
