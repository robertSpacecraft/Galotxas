...
- **REGRESIÓN GLOBAL 7G.2 (EN CURSO / NO-GO):** Se confirmó que el aparente `P0-STAGING-NEWS-DETAIL` fue un falso positivo del smoke test (uso manual de un slug erróneo); el detalle de noticias funciona correctamente en staging. Los advisories npm detectados en la auditoría han sido remediados técnicamente y publicados en `develop` (actualización de `react-router-dom` a `7.18.2`; `npm audit` y `npm audit --omit=dev` en 0). El gate 7G.2 permanece ABIERTO / NO-GO únicamente por el hallazgo independiente del contrato HTTP de `/sitemap.xml` (devuelve 200 SPA shell en lugar de 404 estricto para slugs desconocidos). La auditoría 7G.3 también permanece ABIERTA, la Fase 7G.4 NO está iniciada y Producción continúa sin desplegar.
