...
- **DESPLIEGUE PRODUCTIVO (7G.4, 7G.5, 7G.6 CERRADOS) Y ENTRADA A 7G.7 (EN CURSO):** Se registró GO explícito a producción (7G.4). Se ejecutó exitosamente el despliegue productivo en Railway y Vercel desde `main` en el commit `ec8aba5e29bbc7b5b8d2b361ed12591ba79f1130`, marcando 7G.5 como PASS/CLOSED. Producción se encuentra en estado `live` + `indexing=false` tras detectar un bloqueo SEO por noindex estático al intentar abrir indexación; la corrección técnica ya está implementada y validada localmente, pero falta desplegarla productivamente. El autodeploy de producción desde `main` sigue DESACTIVADO (operación manual) y queda como pendiente operativo. Se superó el smoke productivo mínimo (7G.6 PASS/CLOSED) validando Home, API `/up`, Admin login, páginas CMS del Club recreadas manualmente, Noticias, Colaboradores, Legal y Escuela. La recuperación de contraseña operó extremo a extremo (correo recibido y reset efectivo). El registro público generó cuenta real exitosa (POST 201 y perfil activo). Se registra como mejora post-MVP (P1, no bloqueante) la "verificación de correo tras registro". Fase 7G.7 EN CURSO, todavía NO debe declararse cerrado el MVP ni crear tag/release.
