...
- **REGRESIÓN GLOBAL 7G.2 (PENDIENTE DE REPETICIÓN GLOBAL):** Se confirmó que el aparente `P0-STAGING-NEWS-DETAIL` fue un falso positivo del smoke test (uso manual de un slug erróneo); el detalle de noticias funciona correctamente en staging. Los advisories npm detectados en la auditoría han sido remediados técnicamente y publicados en `develop` (actualización de `react-router-dom` a `7.18.2`; `npm audit` y `npm audit --omit=dev` en 0). El hallazgo independiente del contrato HTTP de `/sitemap.xml` ha quedado CERRADO / REMEDIADO Y VALIDADO EN VERCEL STAGING (devuelve HTTP 404 estricto, sin caer en fallback SPA). El gate 7G.2 queda PENDIENTE de repetición final del gate global sobre el candidato actual. La auditoría 7G.3 permanece ABIERTA, la Fase 7G.4 NO está iniciada y Producción continúa sin desplegar.
